...
// IWYU pragma: private, include "pinocchio/algorithm/cholesky.hpp"
  ...
  namespace cholesky
  {
      ...
      DataTpl<Scalar, Options, JointCollectionTpl> & data)
    {
      ...
      for (int j = model.nv - 1; j >= 0; --j)
      {
        ...
        if (NVT)
          DUt.noalias() =
            U.row(j).segment(j + 1, NVT).transpose().cwiseProduct(D.segment(j + 1, NVT));

        D[j] = M(j, j) - U.row(j).segment(j + 1, NVT).dot(DUt);
        Dinv[j] = Scalar(1) / D[j];
        for (int _i = data.parents_fromRow[(size_t)j]; _i >= 0;
             _i = data.parents_fromRow[(size_t)_i])
      ...
      struct Uv<Mat, 1>
      {
          ...
          const Eigen::MatrixBase<Mat> & v)
        {
          ...
          for (int k = 0; k < model.nv - 1; ++k) // You can stop one step before nv
            v_.row(k) += U.row(k).segment(k + 1, nvt[(size_t)k] - 1)
                         * v_.middleRows(k + 1, nvt[(size_t)k] - 1);
        }
      };
      ...
      struct Utv<Mat, 1>
      {
          ...
          const Eigen::MatrixBase<Mat> & v)
        {
          ...
          for (int k = model.nv - 2; k >= 0; --k) // You can start from nv-2 (no child in nv-1)
            v_.segment(k + 1, nvt[(size_t)k] - 1) +=
              U.row(k).segment(k + 1, nvt[(size_t)k] - 1).transpose() * v_[k];
        }
      };
    ...
    namespace internal
    {
      ...
      struct Uiv<Mat, 1>
      {
          ...
          const Eigen::MatrixBase<Mat> & v)
        {
          ...
          for (int k = model.nv - 2; k >= 0; --k) // You can start from nv-2 (no child in nv-1)
            v_[k] -= U.row(k)
                       .segment(k + 1, nvt[(size_t)k] - 1)
                       .dot(v_.segment(k + 1, nvt[(size_t)k] - 1));
        }
      };
    ...
    namespace internal
    {
        ...
        const Eigen::MatrixBase<VectorLike> & v)
      {
        ...
        const int last_col =
          std::min<int>(col - 1, model.nv - 2); // You can start from nv-2 (no child in nv-1)
