...
namespace pinocchio
{
  ...
  struct log3_impl<CppAD::AD<_Scalar>>
  {
      ...
      const Eigen::MatrixBase<Vector3Out> & res)
    {
      ...
      theta = CppAD::CondExpLt<_Scalar>(
        tr, Scalar(-1), PI_value,
        CppAD::CondExpGt<_Scalar>(
          tr, Scalar(3), Scalar(0), math::acos((tr - Scalar(1)) / Scalar(2))));
  ...
  struct Jlog3_impl<CppAD::AD<_Scalar>>
  {
      ...
      const Eigen::MatrixBase<Matrix3Like> & Jlog)
    {
      ...
      const Scalar alpha = CppAD::CondExpLt<_Scalar>(
        theta, TaylorSeriesExpansion<_Scalar>::template precision<3>(),
  ...
  struct log6_impl<CppAD::AD<_Scalar>>
  {
    ...
    static void run(const SE3Tpl<Scalar, Options> & M, MotionDense<MotionDerived> & mout)
    {
      ...
      alpha = CppAD::CondExpLt<_Scalar>(
        t, TaylorSeriesExpansion<_Scalar>::template precision<3>(),
        Scalar(1) - t2 / Scalar(12) - t2 * t2 / Scalar(720),
        t * st / (Scalar(2) * (Scalar(1) - ct)));
      beta = CppAD::CondExpLt<_Scalar>(
        t, TaylorSeriesExpansion<_Scalar>::template precision<3>(),
      ...
      MotionDense<MotionDerived> & mout)
    {
      ...
      const Scalar pos_neg = CppAD::CondExpGe<_Scalar>(quat.w(), Scalar(0), Scalar(+1), Scalar(-1));
      Scalar theta;
      ...
      const Scalar alpha = CppAD::CondExpLe<_Scalar>(
        theta, TaylorSeriesExpansion<Scalar>::template precision<3>(),
      ...
      const Scalar beta = CppAD::CondExpLe<_Scalar>(
        theta, TaylorSeriesExpansion<Scalar>::template precision<3>(),
  ...
  struct Jlog6_impl<CppAD::AD<_Scalar>>
  {
    ...
    static void run(const SE3Tpl<Scalar, Options> & M, const Eigen::MatrixBase<Matrix6Like> & Jlog)
    {
      ...
      beta = CppAD::CondExpLt<_Scalar>(
        t, TaylorSeriesExpansion<_Scalar>::template precision<3>(),
        Scalar(1) / Scalar(12) + t2 / Scalar(720), t2inv - st * tinv * inv_2_2ct);

      beta_dot_over_theta = CppAD::CondExpLt<_Scalar>(
        t, TaylorSeriesExpansion<_Scalar>::template precision<3>(), Scalar(1) / Scalar(360),
