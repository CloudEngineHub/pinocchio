...
namespace pinocchio
{
    ...
    const Scalar mu)
  {
    ...
    const Eigen::Index constraint_total_size = getTotalConstraintResidualSize(contact_models);
    ...
    for (size_t k = 0; k < contact_models.size(); ++k)
    {
      ...
      {
        const int size = cmodel.residualSize();
        ...
        for (size_t i = 0; i < k; ++i)
        {
          ...
          const int size_other = cmodel_other.residualSize();
    ...
    const Scalar mu)
  {
    ...
    for (size_t k = 0; k < contact_models.size(); ++k)
    {
      ...
      while (an_i > 1)
      {
        if (contact_models[k].residualSize() != 3)
