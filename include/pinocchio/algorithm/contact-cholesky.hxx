...
namespace pinocchio
{
    ...
    const std::vector<ConstraintData, ConstraintDataAllocator> & constraint_datas)
  {
    ...
    const Eigen::Index total_dim = nv + num_total_constraints;
    ...
    /*
              ...
              ++it, ++constraint_id)
          {
            ...
            for(Eigen::Index k = 0; k < contact_dim; ++k)
            {
              ...
              for(Eigen::Index joint1_indexes_constraint_id = num_total_constraints;
                  joint1_indexes_constraint_id < total_dim;
    ...
    bool use_explicit_delassus)
  {
    ...
    const Eigen::Index total_dim = size();
    const Eigen::Index total_constraint_size = total_dim - nv;
    ...
    const Eigen::MatrixBase<VectorLike> & vec, bool use_explicit_delasssus)
  {
    ...
    else
    {
      ...
      for (Eigen::Index j = total_constraint_size - 1; j >= 0; --j)
      {
        const Eigen::Index slice_dim = total_dim - j - 1;
        auto DUt_partial = DUt.head(slice_dim);
        DUt_partial.noalias() =
          U.row(j).segment(j + 1, slice_dim).transpose().cwiseProduct(D.segment(j + 1, slice_dim));
  ...
  namespace details
  {
    ...
    struct UvAlgo<VectorLike, 1>
    {
        ...
        const Eigen::MatrixBase<VectorLike> & vec)
      {
        ...
        const Eigen::Index num_total_constraints = chol.size() - chol.nv;

        // TODO: exploit the Sparsity pattern of the first rows of U
        for (Eigen::Index k = 0; k < num_total_constraints; ++k)
        {
          const Eigen::Index slice_dim = chol.size() - k - 1;
          vec_[k] += chol.U.row(k).tail(slice_dim).dot(vec_.tail(slice_dim));
        }

        for (Eigen::Index k = num_total_constraints; k <= chol.size() - 2; ++k)
          vec_[k] += chol.U.row(k)
  ...
  namespace details
  {
    ...
    struct UtvAlgo<VectorLike, 1>
    {
        ...
        const Eigen::MatrixBase<VectorLike> & vec)
      {
        ...
        PINOCCHIO_CHECK_INPUT_ARGUMENT(
          vec.size() == chol.size(), "The input vector is of wrong size");
        const Eigen::Index num_total_constraints = chol.constraintDim();

        for (Eigen::Index k = chol.size() - 2; k >= num_total_constraints; --k)
          vec_.segment(k + 1, chol.nv_subtree_fromRow[k] - 1) +=
            chol.U.row(k).segment(k + 1, chol.nv_subtree_fromRow[k] - 1).transpose() * vec_[k];

        // TODO: exploit the Sparsity pattern of the first rows of U
        for (Eigen::Index k = num_total_constraints - 1; k >= 0; --k)
        {
          const Eigen::Index slice_dim = chol.size() - k - 1;
