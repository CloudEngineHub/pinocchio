...
#ifndef __pinocchio_math_matrix_info_hpp__
#define __pinocchio_math_matrix_info_hpp__

#include "pinocchio/math/fwd.hpp"

/**
 * @file matrix-info.hpp
 * @brief Defines the pinocchio::MatrixInfo struct, a utility for storing matrix dimensions.
 */
...
namespace pinocchio
{
   ...
   *
   * @details This utility struct is used to store and pass around the size information of a matrix,
   *          particularly when the dimensions need to be queried from an algorithm or function.
   *
   *          A default-constructed instance is initialized to an invalid state (`rows = -1`, `cols
   * = -1`), which can be verified using the isValid() method.
   */
  struct MatrixInfo
  {
...
#endif // ifndef __pinocchio_math_matrix_info_hpp__
