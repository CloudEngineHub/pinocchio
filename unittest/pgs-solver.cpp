...
struct TestBoxTpl
{
  ...
  void operator()(
    ...
    const double abs_prec = 1e-10,
    const double rel_prec = 1e-12,
    const int maxit = 10000)
  {
    ...
    pgs_solver.setAbsolutePrecision(abs_prec);
    ...
    if (test_warmstart)
    {
      ...
      has_converged =
        has_converged
        && pgs_solver.solve(delassus, g, constraint_models, dt, primal_solution_warmstart);
      primal_solution = pgs_solver.getPrimalSolution();
    }
    ...
    n_iter = pgs_solver.getIterationCount();
...
BOOST_AUTO_TEST_CASE(stack_of_boxes)
{
  ...
  // Test static motion with zero external force
  {
    ...
    TestBox test(model, constraint_models);
    test(q0, v0, tau0, fext, dt, false, 1e-10, 1e-10, 100000);
...
BOOST_AUTO_TEST_CASE(joint_limit_slider)
{
  ...
  // External torques push the slider against the lower bound
  {
    ...
    const bool has_converged = pgs_solver.solve(
      G_plain, g_tilde_against_lower_bound, constraint_models, dt, primal_solution_constref);
    primal_solution = pgs_solver.getPrimalSolution();
    BOOST_CHECK(has_converged);
  ...
  // External torques push the slider away from the lower bound
  {
    ...
    const bool has_converged =
      pgs_solver.solve(G_plain, g_tilde_move_away, constraint_models, dt, primal_solution_constref);
    primal_solution = pgs_solver.getPrimalSolution();
    BOOST_CHECK(has_converged);
...
BOOST_AUTO_TEST_CASE(joint_limit_translation)
{
  ...
  // Gravity pushes the freeflyer against the lower bound
  {
    ...
    // pgs_solver.getAbsoluteConvergenceResidual() << std::endl; std::cout << "
    // pgs_solver.getRelativeConvergenceResidual():   " <<
    // pgs_solver.getRelativeConvergenceResidual() << std::endl;
    BOOST_CHECK(has_converged);

    constraint_velocity = G_plain * primal_solution + g_against_lower_bound;
    constraint_velocity /= dt;
    Eigen::VectorXd dual_solution = pgs_solver.getDualSolution();
